...
export function DownloadGeoJSONButton({ filename = "data.geojson" }) {
  const { nsiBbox } = useConnect("selectNsiBbox");
  const [busy, setBusy] = useState(false);

  const handleDownload = async () => {
    if (!nsiBbox.length || busy) return;
    setBusy(true);
    try {
      const response = await fetch(`api/structures?bbox=${nsiBbox[0]}`);
      if (!response.ok) {
